...
try {
  ...
  return (
    <div className="bg-black rounded-lg overflow-hidden shadow-xl p-6">
      <div className="mb-6">
        ...
        <div className="flex justify-between text-sm text-gray-400">
          ...
          <span>TOTAL MANAGERS: {managers.length}</span>
        </div>
      </div>

      <div className="flex">
        {/* Manager Names (Left Side) */}
        <div className="w-32 mr-4">
          <div className="h-12 flex items-center text-xs font-bold text-gray-400 border-b border-gray-700">
            MANAGER
          </div>
          {managers.map((manager, index) => (
            <div 
              key={manager} 
              className="h-6 flex items-center text-xs text-white border-b border-gray-800"
              style={{ color: colors[index % colors.length] }}
            >
              {manager.length > 10 ? manager.substring(0, 10) + '...' : manager}
            </div>
          ))}
        </div>
        ...
        {/* Current Positions (Right Side) */}
        <div className="w-24 ml-4">
          <div className="h-12 flex items-center justify-center text-xs font-bold text-gray-400 border-b border-gray-700">
            CURRENT
          </div>
          {managers.map((manager, index) => {
            const currentPosition = chartData[1] ? chartData[1][manager] : chartData[0]?.[manager];
            ...
            return (
              <div 
                key={manager} 
                className="h-6 flex items-center justify-center text-xs text-white border-b border-gray-800"
                style={{ color: colors[index % colors.length] }}
              >
                {currentPosition}{positionSuffix}
              </div>
            );
          })}
        </div>
